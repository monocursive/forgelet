defmodule Forgelet.Agent.Workspace do
  @moduledoc """
  ...
  alias Forgelet.{Git, Identity, Repository}
  ...
  def create_session_root do
    base =
      Application.get_env(
        ...
        Path.join(System.tmp_dir!(), "forgelet-sessions")
      )
    ...
    with {:ok, repo_path} <- ensure_repo_checkout(session_root, repo_id),
         :ok <-
           Git.configure_identity(
             repo_path,
             "Forgelet #{fingerprint}",
             "#{fingerprint}@forgelet.local"
           ),
