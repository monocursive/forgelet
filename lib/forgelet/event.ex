defmodule Forgelet.Event do
  @moduledoc """
  Immutable, cryptographically signed events that form the Forgelet event log.
  ...
  def new(kind, keypair, payload, opts \\ []) do
    if kind not in @valid_kinds do
      ...
      signature = Forgelet.Identity.sign(bytes, keypair.secret)
  ...
  def verify(%__MODULE__{} = event) do
    ...
    cond do
      ...
      not Forgelet.Identity.verify(bytes, event.signature, event.author) ->
