defmodule ForgeletWeb.MCP.Plug do
  @moduledoc """
  HTTP transport endpoint for Forgelet MCP JSON-RPC requests.
  """

  import Plug.Conn

  alias Forgelet.Agent.SessionRegistry
  alias ForgeletWeb.MCP.{JsonRpc, ToolRegistry}
  ...
  defp dispatch(%{id: id, method: "initialize"}, _session, _token) do
    JsonRpc.result(id, %{
      ...
      "capabilities" => %{"tools" => %{}},
      "serverInfo" => %{"name" => "forgelet", "version" => "0.1.0"}
    })
  end
